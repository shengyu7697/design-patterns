...
## 靜態工廠方法
C++ 中的靜態工廠方法（Static Factory Method）是一種設計模式，用於建立物件而無需直接使用建構子。它是一個靜態方法，通常定義在類別內部，負責根據不同的條件或參數建立並回傳類型的實例。  
...
```
在這個範例中，ProductFactory 類中的靜態方法 createProduct 根據傳入的參數（如 "A" 或 "B"）來建立並回傳不同類型的 Product 物件。這樣，客戶端程式碼無需知道具體的物件是如何建立的，只需透過工廠方法請求所需的物件。  
...
**工廠模式**  
工廠模式是一種更為複雜的設計模式，通常包括兩種類型：簡單工廠模式和工廠方法模式。  
